...
def bind_gma_contract(contract: bytes) -> GmaContractBinding:
    ...
    try:
        ...
        raise ValueError("invalid GMA contract") from None
    if producer != _PRODUCER:
        raise ValueError("GMA producer identity is required")
    ...
    if not isinstance(revision, str) or _COMMIT.fullmatch(revision) is None:
        raise ValueError("GMA revision must be immutable")
    if not isinstance(digest, str) or _DIGEST.fullmatch(digest) is None:
        raise ValueError("GMA object digest is invalid")
    fields = ("dataset", "path")
    if any(not isinstance(location.get(field), str) or not location[field] for field in fields):
        raise ValueError("GMA location is invalid")
    if any(not isinstance(source.get(field), str) or not source[field] for field in ("source_id", "layer")):
        raise ValueError("GMA source identity is invalid")
    return GmaContractBinding(producer, location["dataset"], revision, location["path"], digest, source["source_id"], source["layer"])
