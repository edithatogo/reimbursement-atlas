...
def build_final_handoff_tasks(root: Path | None = None) -> list[FinalHandoffTaskRecord]:
    ...
    tasks = [
        FinalHandoffTaskRecord(
            ...
            required_environment="Network-enabled local machine or GitHub runner with outbound HTTPS.",
            command="PYTHONPATH=src reimbursement-atlas source-download-plan --method curl && bash data/derived/source_downloads/download_commands.sh",
            evidence_path="data/derived/source_downloads/download_attempts.jsonl",
            unblock_condition="MBS/PBS/CMS source hosts resolve and downloads complete into ignored data/raw_live storage.",
        ...
        FinalHandoffTaskRecord(
            ...
            required_environment="Local checkout with downloaded MBS item-map and descriptor TXT files.",
            command="PYTHONPATH=src reimbursement-atlas reviewed-mbs-txt-pair-bundle data/raw_live/au_mbs/20260701_MBSONLINE_IMAP.TXT data/raw_live/au_mbs/20260701_MBSONLINE_DESC.TXT",
            ...
            recommended_action="Commit only derived rows and redacted provenance after licence review.",
        ),
        FinalHandoffTaskRecord(
            ...
            required_environment="Human review of AMA/CPT licence implications before parsing or publishing fields.",
            command="PYTHONPATH=src reimbursement-atlas source-files",
            evidence_path="docs/LIVE_SOURCE_VALIDATION_PLAYBOOK.md",
            unblock_condition="Reviewer approves the set of derived CLFS fields that can be stored and published.",
            recommended_action="Avoid CPT descriptor redistribution; publish payment/metadata only when permitted.",
        ),
        FinalHandoffTaskRecord(
            ...
            status="blocked_network",
            required_environment="Network-enabled Python environment with access to PyPI advisory APIs.",
            command="uv run --all-extras pip-audit --strict",
            evidence_path="data/derived/external_quality_gates.json",
            unblock_condition="PyPI advisory lookup completes with no unresolved vulnerabilities.",
        ...
        FinalHandoffTaskRecord(
            ...
            unblock_condition="All action references are resolved and workflows use immutable SHAs where practical.",
        ...
        FinalHandoffTaskRecord(
            ...
            required_environment="GitHub Actions with HF_TOKEN, HF_DATASET_REPO and HF_SPACE_REPO secrets.",
            command="gh workflow run huggingface.yml",
            evidence_path=".github/workflows/huggingface.yml",
            unblock_condition="Secrets are configured and publication manifest has no raw-source warnings.",
            recommended_action="Run as a dry release first; inspect dataset card and Space build logs.",
        ),
        FinalHandoffTaskRecord(
            ...
            recommended_action="Treat OSF upload as preregistration candidate, not final publication, until approved.",
        ),
        FinalHandoffTaskRecord(
            ...
            status="blocked_review",
            required_environment="Network-enabled runner after source/security/publication gates complete.",
            command="PYTHONPATH=src reimbursement-atlas release-readiness && PYTHONPATH=src reimbursement-atlas data-quality",
            ...
            recommended_action="Only then cut a signed, attested public release and archive to Zenodo/OSF.",
        ),
    ]
    return tasks
...
def write_final_handoff_tasks(
    ...
    *,
    ...
    summary = {
        ...
        "download_ready": True,
        "download_ready_note": "Repo archive is ready; remaining tasks require network, credentials, or human licence/research review.",
    }
