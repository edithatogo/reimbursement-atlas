# Protocol: cognitive/procedural fee relativities
...
## Datasets
Primary datasets are: Australia MBS professional services, US CMS Physician Fee Schedule, Canadian provincial physician schedules, NHS/NZA or other tariffs where the professional/facility split is interpretable.
...
The unit of comparison for this question is clinical work unit, professional versus facility component, consultation duration and procedural bundle boundary. Mappings are allowed only as candidate relationships until adjudicated. One-to-many and many-to-one mappings must be represented explicitly rather than collapsed into a false one-to-one equivalence.
...
## Outputs
Planned outputs are relativity indices, sensitivity ranges for bundled components, basket definitions and a policy-facing explanation of incentives. Public outputs must pass the publication manifest, data-quality gate, public-data policy, evidence-readiness gate and source-drift gate. Dashboards should present caveats next to metrics rather than separating them into appendices only.
