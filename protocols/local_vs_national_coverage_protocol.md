# Protocol: local versus national coverage discretion
...
## Datasets
Primary datasets are: CMS NCD/LCD/article records, MBS/PBS/MSAC/PBAC restrictions, NHS/NICE/genomics coverage sources and selected provincial/state coverage records.
...
The unit of comparison for this question is coverage decision level, local jurisdiction, eligibility language and implementation channel. Mappings are allowed only as candidate relationships until adjudicated. One-to-many and many-to-one mappings must be represented explicitly rather than collapsed into a false one-to-one equivalence.
...
## Outputs
Planned outputs are coverage-discretion map, local/national typology, caveated access-equity interpretation and policy brief. Public outputs must pass the publication manifest, data-quality gate, public-data policy, evidence-readiness gate and source-drift gate. Dashboards should present caveats next to metrics rather than separating them into appendices only.
