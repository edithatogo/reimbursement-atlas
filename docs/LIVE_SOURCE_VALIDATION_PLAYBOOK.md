...
| PBS | PBS API is the current distribution mechanism; the official catalogue identifies the v3 server, OpenAPI export, `/schedules`, `/items` and `/fees` operations, and JSON/CSV responses. The catalogue publishes a public-user subscription key. | Copy the current key from the official catalogue only at runtime, fetch `schedules` with `Accept: application/json` before selecting a monthly `schedule_code`, inspect rate-limit headers, then retain only a reviewed JSON/CSV extract and redacted provenance. |
...
The repo should not yet perform broad scheduled fetches, scrape PDFs at scale, publish raw restricted terminology packages, or infer net drug prices from published list/payment-limit fields. Live ingestion should remain manually reviewed until at least the first MBS, PBS, CLFS, PFS and ASP validations are complete. The PBS documentation page and OpenAPI export are not themselves monthly extracts: the public catalogue key enables acquisition, but human field/licence review remains a publication prerequisite.
