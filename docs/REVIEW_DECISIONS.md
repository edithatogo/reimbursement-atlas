...
| OSF protocol package | prepared, unpublished | `data/derived/osf/sync_manifest.jsonl` | Confirm target project and approve rows before changing `publish_allowed`. |
| Hugging Face dataset/Space | validated, unpublished | `scripts/check_huggingface_bundle.py` | Configure targets/secrets and explicitly approve remote publication. |
...
| Dashboard visual baselines | pending visual review | `docs/DASHBOARD_VALIDATION.md` | Review supported browser/OS screenshots; do not treat macOS smoke evidence as cross-platform approval. |
